...
from enre.vis.lsif import to_lsif
...
def main() -> None:
    ...
    parser.add_argument("--output", default="json",choices=["json", "lsif"], help="output format: json or lsif")
    ...
    start = time.time()
    manager = enre_wrapper(root_path, config.output, config.compatible, config.cfg, config.cg, config.builtins)
    end = time.time()
...
def enre_wrapper(root_path: Path, output_format: str, compatible_format: bool, need_cfg: bool, need_call_graph: bool,
    ...
    manager.work_flow()
    out_path = f"{project_name}-report-enre"
    if need_cfg:
